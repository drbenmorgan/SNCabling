...
class CaloSignalBoardAddress :
    
    def __init__(self, label_):
        self.label = label_.strip()
        self.addrtype = self.label[0]
        if self.addrtype != "B" :
            raise Exception("Invalid CaloSignal board address '%s'!" % self.label)
...
class CaloSignalChannelAddress :
...
class CaloSignalExternalCableAddress :
    
    def __init__(self, label_):
        self.label = label_.strip()
        self.addrtype = self.label[0]
        if self.addrtype != "L" :
            raise Exception("Invalid CaloSignal external cable address '%s'!" % self.label)
        address = self.label.split(':')[1].strip()
        self.address = address.split('.')
        self.harness = 666
        self.cable   = 666
    ...
    def to_string(self):
        return "L:{:d}.{:d}".format(self.harness, self.cable)
...
class CaloSignalInternalCableAddress :
    
    def __init__(self, label_):
        self.label = label_.strip()
        self.addrtype = self.label[0]
        if self.addrtype != "A" :
            raise Exception("Invalid CaloSignal internal cable address '%s'!" % self.label)
        address = self.label.split(':')[1].strip()
        self.address = address.split('.')
        self.harness = 666
        self.cable   = 666
...
class CaloSignalExternalHarnessAddress :
    
    def __init__(self, label_):
        self.label = label_.strip()
        self.addrtype = self.label[0]
        if self.addrtype != "E" :
            raise Exception("Invalid CaloSignal external harness address '%s'!" % self.label)
        address = self.label.split(':')[1].strip()
        self.address = address.split('.')
        self.harness = 666
        if self.address[0] != '?' :
...
class CaloSignalInternalHarnessAddress :
    
    def __init__(self, label_):
        self.label = label_.strip()
        self.addrtype = self.label[0]
        if self.addrtype != "I" :
            raise Exception("Invalid CaloSignal internal harness address '%s'!" % self.label)
        address = self.label.split(':')[1].strip()
        self.address = address.split('.')
        self.harness = 666
        if self.address[0] != '?' :
